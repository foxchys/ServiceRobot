...
int main(int argc, const char** argv)
{
	...
	while (1)
	{
		...
		if  (i<faces.size())
		{
			...
			//cv::Point center(faces[i].x + faces[i].width*0.5, faces[i].y + faces[i].height*0.5);//计算面部中心
			//ellipse(frame, center, cv::Size(faces[i].width*0.5, faces[i].height*0.5), 0, 0, 360, cv::Scalar(0, 255, 0), 4, 8, 0);//在面部画圆
